    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed):
        ...
        if self._state == FOLLOW_LANE:
            ...
            if stop_sign_found:
                ...
                print('Switch to DECELERATE_TO_STOP model')
            # ------------------------------------------------------------------
        ...
        # state.
            ...
            if closed_loop_speed < STOP_THRESHOLD:
                print('Switch to STAY_STOPPED model')
        ...
        # the stop sign and transition to the next state.
            ...
            if self._stop_count == STOP_COUNTS:
                ...
                if not stop_sign_found:
                    self._stop_count = 0
                    self._state = FOLLOW_LANE
                    print('Switch to FOLLOW_LANE model')
                # --------------------------------------------------------------
...
def get_closest_index(waypoints, ego_state):
    ...
    for i in range(len(waypoints)):
        waypoint_local_x = (waypoints[i][0]- ego_state[0])*math.cos(ego_state[2])+ (waypoints[i][1] - ego_state[1])*math.sin(ego_state[2])
        waypoint_local_y = -(waypoints[i][0] - ego_state[0])*math.sin(ego_state[2]) + (waypoints[i][1] - ego_state[1])*math.cos(ego_state[2])
        if waypoint_local_x < 0:
            continue
        else:
            dist = math.sqrt(waypoint_local_x**2 + waypoint_local_y**2)
            if closest_len > dist:
                closest_index = i
                closest_len = dist
    # ------------------------------------------------------------------
