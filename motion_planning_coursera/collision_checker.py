        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                for point in range(len(path[0])):
                    score += sqrt((path[0][point]- goal_state[0])**2+ (path[1][point]- goal_state[1])**2)/len(path[0])
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]: # means j path has collision
                            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                            # --------------------------------------------------
                            for point in range(len(paths[j][0])):
                                score += self._weight *  sqrt( (paths[i][0][point]-paths[j][0][point])**2 
                                                                    + (paths[i][1][point]-paths[j][1][point])**2 ) /len(paths[j][0])
